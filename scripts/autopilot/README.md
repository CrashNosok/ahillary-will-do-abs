...
Оркестратор двигает карточку и логирует; сам цикл разработки задаёт `docs/review-playbook.md`
(v3.0): **0 preflight → 1 прочитать → 2 ПЛАН (изучить код + 5-7 UI-сценариев) → 3 реализация
+ гейты → 4 РЕВЬЮ (`/code-review` + `ecc:security-review`, починить ВСЕ находки) → 5 живой
Playwright по 5-7 сценариям ТОЛЬКО через UI → 6 коммит + финальные сценарии**. Ни один шаг не
пропускается.
...
|---|---|---|
| `--pw-mode` | `full` | полный цикл: гейты + ревью-скиллы + живой Playwright по 5-7 UI-сценариям |
