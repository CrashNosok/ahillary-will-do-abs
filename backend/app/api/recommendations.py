...
# Платная генерация отчёта — не чаще раза в это окно на пользователя (иначе 429).
_GENERATE_COOLDOWN = dt.timedelta(minutes=5)
...
def create_recommendation(
    ...
    enforce_cooldown(latest.created_at if latest else None, _GENERATE_COOLDOWN)
    try:
